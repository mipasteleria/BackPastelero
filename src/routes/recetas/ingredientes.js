...
router.put('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const ingrediente = req.body;
    const updatedIngrediente = await Ingrediente.findByIdAndUpdate(id, ingrediente, { new: true });
    if (!updatedIngrediente) {
      return res.status(404).send({ message: 'Ingrediente no encontrado' });
    }
    res.status(200).send({ message: 'Ingrediente actualizado', data: updatedIngrediente });
  } catch (error) {
    res.status(400).send({ message: error.message });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    res.status(200).send({ message: 'Ingrediente eliminado', data: deletedIngrediente });
  } catch (error) {
    res.status(400).send({ message: error.message });
  }
});
